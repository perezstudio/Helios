...
struct PermissionRequestView: View {
	...
	var body: some View {
		VStack(alignment: .leading, spacing: 12) {
			...
			HStack(spacing: 12) {
				Button("Allow") {
					print("Permission allowed: \(request.permission.rawValue) for \(request.domain)")
				...
				Button("Block") {
					print("Permission blocked: \(request.permission.rawValue) for \(request.domain)")
		...
		.shadow(radius: 4)
		.onAppear {
			print("PermissionRequestView appeared for: \(request.domain) - \(request.permission.rawValue)")
		}
	}
}
